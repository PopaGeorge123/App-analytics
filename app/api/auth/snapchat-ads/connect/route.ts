...
import { handleSnapchatAdsConnect }  from "@/lib/integrations/snapchat-ads/callback";
...
export async function POST(request: NextRequest) {
  ...
  try {
    await handleSnapchatAdsConnect(user.id, accessToken, accountId);
    await notifyIntegrationConnected(user.id, "snapchat-ads");

    return NextResponse.json({ success: true });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
    console.error("Snapchat Ads connect error:", err);
