...
export async function GET(request: NextRequest) {
  ...
  if (error || !code || !state) {
    return NextResponse.redirect(
      new URL("/dashboard?tab=data-sources&convertkit=error", process.env.NEXT_PUBLIC_APP_URL),
    );
  }
  try {
    await handleConvertKitOAuthCallback(state, code);
    await notifyIntegrationConnected(state, "convertkit");
    return NextResponse.redirect(
      new URL("/dashboard?tab=data-sources&convertkit=connected&syncing=convertkit", process.env.NEXT_PUBLIC_APP_URL),
    );
  } catch (err) {
