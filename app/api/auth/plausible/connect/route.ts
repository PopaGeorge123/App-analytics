...
import { handlePlausibleConnect }    from "@/lib/integrations/plausible/callback";
...
export async function POST(request: NextRequest) {
  ...
  try {
    await handlePlausibleConnect(user.id, apiKey, siteId);
    await notifyIntegrationConnected(user.id, "plausible");

    return NextResponse.json({ success: true });
  } catch (err) {
    const message = err instanceof Error ? err.message : "Unknown error";
    console.error("Plausible connect error:", err);
