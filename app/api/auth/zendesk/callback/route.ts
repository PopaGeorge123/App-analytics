...
export async function GET(request: NextRequest) {
  ...
  if (error || !code || !userId || !subdomain) {
    return NextResponse.redirect(
      new URL("/dashboard?tab=data-sources&zendesk=error", process.env.NEXT_PUBLIC_APP_URL),
    );
  }
  try {
    await handleZendeskOAuthCallback(userId, code, subdomain);
    return NextResponse.redirect(
      new URL("/dashboard?tab=data-sources&zendesk=connected&syncing=zendesk", process.env.NEXT_PUBLIC_APP_URL),
    );
  } catch (err) {
