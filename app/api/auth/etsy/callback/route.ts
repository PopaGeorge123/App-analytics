...
export async function GET(request: NextRequest) {
  ...
  if (error || !code || !state) {
    return NextResponse.redirect(
      new URL("/dashboard?tab=data-sources&etsy=error", process.env.NEXT_PUBLIC_APP_URL),
    );
  }
  try {
    await handleEtsyOAuthCallback(state, code, codeVerifier);
    const response = NextResponse.redirect(
      new URL("/dashboard?tab=data-sources&etsy=connected&syncing=etsy", process.env.NEXT_PUBLIC_APP_URL),
    );
