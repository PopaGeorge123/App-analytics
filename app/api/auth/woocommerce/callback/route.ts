...
export async function GET(request: NextRequest) {
  ...
  if (error || !code || !state) {
    return NextResponse.redirect(
      new URL("/dashboard?tab=data-sources&woocommerce=error", process.env.NEXT_PUBLIC_APP_URL),
    );
  }
  try {
    await handleWooCommerceOAuthCallback(state, code);
    await notifyIntegrationConnected(state, "woocommerce");
    return NextResponse.redirect(
      new URL("/dashboard?tab=data-sources&woocommerce=connected&syncing=woocommerce", process.env.NEXT_PUBLIC_APP_URL),
    );
  } catch (err) {
