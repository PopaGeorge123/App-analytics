...
export async function GET(request: NextRequest) {
  ...
  if (error || !code || !state) {
    return NextResponse.redirect(
      new URL("/dashboard?tab=data-sources&linkedin-ads=error", process.env.NEXT_PUBLIC_APP_URL),
    );
  }
  try {
    await handleLinkedInAdsOAuthCallback(state, code);
    await notifyIntegrationConnected(state, "linkedin-ads");
    return NextResponse.redirect(
      new URL("/dashboard?tab=data-sources&linkedin-ads=connected&syncing=linkedin-ads", process.env.NEXT_PUBLIC_APP_URL),
    );
  } catch (err) {
