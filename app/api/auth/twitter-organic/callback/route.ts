...
export async function GET(request: NextRequest) {
  ...
  if (error || !code || !state) {
    return NextResponse.redirect(
      new URL("/dashboard?tab=data-sources&twitter-organic=error", process.env.NEXT_PUBLIC_APP_URL),
    );
  }
  try {
    await handleTwitterOrganicOAuthCallback(state, code, codeVerifier);
    const response = NextResponse.redirect(
      new URL("/dashboard?tab=data-sources&twitter-organic=connected&syncing=twitter-organic", process.env.NEXT_PUBLIC_APP_URL),
    );
