  ...
  return (
    <div className="relative flex items-center justify-center">
      ...
      <div className="absolute flex flex-col items-center">
        ...
        <span className="font-mono text-[10px] uppercase tracking-widest text-[#4a4a6a]">/ 100</span>
...
function TaskCard({
  ...
  const cat = CATEGORY_META[task.category] ?? { label: task.category, color: "#8888aa", icon: "•" };
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl border transition-all duration-300 ${
        task.completed
          ? "border-[#1a1a28] bg-[#0d0d16]/50 opacity-55"
          : confirming
          ? "border-[#f59e0b]/30 bg-[#13120a]"
          : "border-[#1e1e2e] bg-[#111119] hover:border-[#2a2a3e] hover:bg-[#121220]"
      }`}
    >
      {/* ── Colour accent bar (left edge) ── */}
      <div
        className="absolute left-0 top-0 bottom-0 w-0.75 rounded-l-2xl"
        style={{ backgroundColor: task.completed ? "#2a2a3e" : cat.color }}
      />
      ...
      <div className="pl-5 pr-4 py-4">
        ...
        <div className="flex items-center justify-between gap-3 mb-3">
          <span
            ...
            style={{
              color: task.completed ? "#4a4a6a" : cat.color,
              backgroundColor: task.completed ? "#1a1a28" : `${cat.color}14`,
            }}
          >
            {cat.icon} {cat.label}
          </span>
            ...
            <button
              ...
              className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border transition-all ${
                task.completed
                  ...
                  : completing
                  ? "border-[#2e2e4e] bg-[#1e1e2e] animate-pulse cursor-wait"
                  : confirming
            ...
            >
              {task.completed && (
                <svg width="11" height="11" viewBox="0 0 10 10" fill="none">
                  <path d="M1.5 5l2.5 2.5 4.5-5" stroke="#0d0d16" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              )}
        ...
        {/* ── Description ── */}
        <p className={`text-[13px] leading-relaxed ${task.completed ? "text-[#2e2e4a]" : "text-[#7070a0]"}`}>
          {task.description}
        </p>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 rounded-xl border border-[#f59e0b]/20 bg-[#f59e0b]/5 px-3 py-2.5">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="rounded-lg bg-[#00d4aa] px-3 py-1.5 font-mono text-[10px] font-bold text-[#0d0d16] transition hover:bg-[#00d4aa]/85"
              >
                Yes, done ✓
              </button>
              <button
                onClick={() => setConfirming(false)}
                className="rounded-lg border border-[#2e2e4e] px-3 py-1.5 font-mono text-[10px] text-[#6666888] transition hover:text-[#f0f0f5] hover:border-[#4a4a6a]"
              >
                Cancel
              </button>
            </div>
...
function AnalyzingSkeleton({ url }: { url: string }) {
  ...
  return (
    <div className="rounded-2xl border border-[#1e1e2e] bg-[#0d0d16]/60 p-8 text-center space-y-6">
      ...
      <div>
        <p className="font-mono text-sm font-semibold text-[#f0f0f5]">Analyzing your website…</p>
        <p className="mt-1 font-mono text-[11px] text-[#4a4a6a] truncate max-w-xs mx-auto">{url}</p>
      </div>
      ...
      <div className="text-left space-y-2 max-w-sm mx-auto">
        {steps.map((s, i) => (
          ...
          >
            ...
            <span className="font-mono text-[11px] text-[#4a4a6a]">{s.label}</span>
          </div>
        ))}
      </div>
...
export default function WebsiteTab({
  ...
  return (
    <div className="max-w-5xl space-y-8">
        ...
        <div className="flex gap-3">
          <div className="flex-1 relative">
            <div className="pointer-events-none absolute inset-y-0 left-3 flex items-center">
              <span className="font-mono text-xs text-[#4a4a6a]">https://</span>
            </div>
            <input
              ref={inputRef}
              type="text"
      ...
      {/* ── State: no URL ─────────────────────────────────────── */}
      {!hasSavedUrl && (
        <div className="rounded-2xl border border-dashed border-[#1e1e2e] p-10 text-center">
          <p className="text-2xl mb-3">🌐</p>
          <p className="font-mono text-sm text-[#4a4a6a] mb-4">
      ...
      {hasSavedUrl && status === "idle" && tasks.length === 0 && (
        <div className="rounded-2xl border border-[#1e1e2e] bg-[#0d0d16]/60 p-10 text-center space-y-4">
          <p className="text-3xl">🤖</p>
          <div>
            <p className="font-mono text-sm font-semibold text-[#f0f0f5]">Ready to analyze</p>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-[#00d4aa] px-6 py-3 font-mono text-xs font-semibold text-[#0d0d16] transition hover:bg-[#00d4aa]/90"
          >
      ...
      {status === "error" && (
        <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-6 space-y-3">
          ...
          {profile.analysis_error && (
            <p className="font-mono text-xs text-[#4a4a6a]">{profile.analysis_error}</p>
          )}
          <button
            onClick={handleAnalyze}
            className="font-mono text-xs font-semibold text-[#00d4aa] hover:underline"
        ...
        <div className="space-y-6">
          ...
          >
            <div className="flex flex-col sm:flex-row items-center gap-6">
              ...
              <div className="flex-1 space-y-3 text-center sm:text-left">
                {profile.description && (
                  <p className="text-sm leading-relaxed text-[#8888aa]">{profile.description}</p>
                )}
                <div>
                  <div className="flex justify-between mb-1">
                    <span className="font-mono text-[10px] text-[#4a4a6a]">Progress to 100</span>
          ...
          {reAnalyzeMsg && (
            <div className="rounded-xl border border-[#00d4aa]/20 bg-[#00d4aa]/5 px-4 py-3 flex items-center justify-between gap-3">
              ...
              <button
                onClick={handleAnalyze}
                className="shrink-0 rounded-lg bg-[#00d4aa] px-4 py-1.5 font-mono text-[11px] font-semibold text-[#0d0d16] hover:bg-[#00d4aa]/90"
              >
                New Analysis
              </button>
            </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                <button
                  ...
                  className={`rounded-full border px-2.5 py-1 font-mono text-[9px] font-semibold uppercase tracking-widest transition-colors ${
                    filterCategory === "all"
                      ? "border-[#8888aa]/40 bg-[#8888aa]/10 text-[#8888aa]"
                      : "border-[#1e1e2e] text-[#4a4a6a] hover:text-[#8888aa]"
                  }`}
                >
                  All types
                </button>
                {activeCategories.map((cat) => {
                  const meta = CATEGORY_META[cat] ?? { label: cat, color: "#8888aa", icon: "•" };
                  ...
                  return (
                    <button
                      ...
                      style={{
                        color: active ? meta.color : "#4a4a6a",
                        borderColor: active ? `${meta.color}50` : "#1e1e2e",
          ...
          {filteredPending.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <p className="font-mono text-[9px] uppercase tracking-widest text-[#4a4a6a]">
          ...
          {filteredDone.length > 0 && (
            <details className="group" open={filterStatus === "done"}>
              <summary className="flex cursor-pointer items-center gap-2 list-none">
                ...
                <div className="flex-1 border-t border-[#1e1e2e]/50" />
                <svg
          ...
          {filteredTasks.length === 0 && tasks.length > 0 && (
            <div className="rounded-xl border border-dashed border-[#1e1e2e] p-6 text-center">
              <p className="font-mono text-xs text-[#4a4a6a]">No tasks match the current filter.</p>
              <button
