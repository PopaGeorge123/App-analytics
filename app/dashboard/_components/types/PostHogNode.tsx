...
export default function PostHogNode({ isConnected = false }: { isConnected?: boolean }) {
    ...
    <div style={{ width: 220, fontFamily: 'sans-serif' }}
      className="border border-[#ffb300] rounded-2xl bg-white overflow-hidden"
    >
      {/* Header */}
      <div className="p-4 border-b border-gray-100">
        <div className="flex items-center gap-2.5 mb-3">
          <div className="w-9 h-9 rounded-lg bg-[#F4F6F0] flex items-center justify-center">
            <img src="/integrations/posthog.svg" alt="PostHog" className="w-5 h-5" />
          </div>
          <div>
            <p className="text-sm font-medium text-gray-900 m-0">PostHog</p>
        ...
        <p className="text-[12px] text-gray-500 leading-relaxed m-0">
          Sync your PostHog data to track website traffic, user behavior, and conversion metrics in real time.
        </p>
      </div>
      ...
      {/* CTA */}
      <div className="p-3">
        <button className="w-full py-2 bg-[#ffb300] text-white text-[13px] font-medium rounded-lg">
          {isConnected ? "Disconnect PostHog" : "Connect PostHog"}
        </button>
      </div>
