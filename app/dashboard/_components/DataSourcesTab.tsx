...
const UI_INTEGRATIONS = LIVE_INTEGRATIONS.map((cat) => ({
  ...
  color: cat.color,
  icon: <img src={cat.icon} alt={cat.name} width={14} height={14} className="object-contain" />,
...
function Toggle({
  checked, onChange, disabled, color = "#10b981",
...
function SectionLabel({ color, children }: { color: string; children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2 mb-1">
      ...
      <p className="font-mono text-[9px] font-bold uppercase tracking-[0.15em]" style={{ color }}>{children}</p>
    </div>
  );
}
...
function IntegrationRow({
...
}: { integration: (typeof UI_INTEGRATIONS)[number]; connected: boolean }) {
  ...
  return (
    <div className="rounded-xl bg-white ring-1 ring-black/[0.06] p-4 transition-all hover:ring-black/[0.12] hover:shadow-[0_2px_8px_rgba(0,0,0,0.06)] shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
      <div className="flex items-start justify-between gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {connected ? (
                <span className="inline-flex items-center gap-1 font-mono text-[9px] font-semibold px-1.5 py-0.5 rounded-md" style={{ color: "#10b981", backgroundColor: "#10b98118" }}>
                  <span className="h-1.5 w-1.5 rounded-full bg-[#10b981]" />Connected
                </span>
              ) : (
                <span className="inline-flex items-center gap-1 font-mono text-[9px] text-[#58588a]">
                  <span className="h-1.5 w-1.5 rounded-full bg-[#d0d0e0]" />Not connected
                </span>
              )}
            </div>
            ...
            {showTooltip && !connected && (
              <div className="mt-1.5 rounded-lg border border-[#14b8a6]/20 bg-[#14b8a6]/5 px-2.5 py-1.5">
                <p className="font-mono text-[9px] text-[#14b8a6]">Unlocks data from {integration.name} in your Overview & AI Advisor</p>
              </div>
            )}
          </div>
        </div>
        {connected ? (
          <div className="flex items-center gap-1.5 shrink-0">
            <button
              onClick={handleSync} disabled={loading === "sync"} title="Sync now"
            ...
            <a
              href={integration.connectUrl} onClick={handleConnectClick}
            ...
            {confirmDisconnect ? (
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => setConfirmDisconnect(false)} className="rounded-lg border border-[#eeeef4] px-2.5 py-1.5 font-mono text-[10px] text-[#6a6a90] hover:text-[#4a4a6a] transition-all">No</button>
              </div>
            ) : (
...
function AlertsSection({ email, currencies }: { email: string; currencies: Record<string, string> }) {
  ...
  const alertRows = [
    {
      key: "revenueDropPct" as keyof AlertRules, toggleKey: "revenue" as const,
      label: "Revenue drop alert", desc: "Alert if 7-day revenue drops by X% vs previous week",
      unit: "%", unitPos: "right" as const,
      suggestion: "Suggested: 20% for early-stage founders",
      iconColor: "#ef4444",
      icon: <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><polyline points="23 18 13.5 8.5 8.5 13.5 1 6" /><polyline points="17 18 23 18 23 12" /></svg>,
    },
    {
      key: "bounceSpikeThreshold" as keyof AlertRules, toggleKey: "bounce" as const,
      label: "Bounce rate spike", desc: "Alert if 7-day average bounce rate exceeds X%",
      unit: "%", unitPos: "right" as const,
      suggestion: "Suggested: 70% — typical healthy sites stay under 60%",
      iconColor: "#f59e0b",
      icon: <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10" /><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10" /></svg>,
    },
    {
      key: "spendSpikeThreshold" as keyof AlertRules, toggleKey: "spend" as const,
      label: "Ad spend cap", desc: `Alert if a single day's ad spend exceeds ${adsCurrency} X`,
      unit: adsCurrency, unitPos: "left" as const,
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-2 rounded-lg border border-[#3b82f6]/20 bg-[#3b82f6]/5 px-3 py-2">
        ...
        <p className="font-mono text-[10px] text-[#3b82f6]">Alerts delivered to: <span className="font-semibold">{email}</span></p>
      </div>
      <div className="space-y-3">
          ...
          return (
            <div key={row.key} className="rounded-xl bg-white ring-1 ring-black/[0.06] overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
              <div className="flex items-center gap-3 px-4 py-3">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg" style={{ backgroundColor: `${row.iconColor}15`, color: row.iconColor }}>{row.icon}</div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-[#3a3a58]">{row.label}</p>
                  <p className="font-mono text-[9px] text-[#6a6a90]">{row.desc}</p>
                </div>
                <Toggle checked={isOn} onChange={(v) => setToggles((t) => ({ ...t, [row.toggleKey]: v }))} color="#10b981" />
              </div>
              {isOn && (
                <div className="border-t border-[#eeeef4] px-4 py-3 bg-[#f5f5f7]/40 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      type="number" min={0} max={row.unit === "%" ? 100 : undefined} placeholder="0"
                      value={val || ""}
                  ...
                  {isOn && val === 0 && (
                    <p className="font-mono text-[10px] text-[#f59e0b] flex items-center gap-1">
                      ...
                      Set to 0 — alert disabled. Enter a value to enable.
                    </p>
                  )}
        ...
        <div className="rounded-xl bg-white ring-1 ring-black/[0.06] px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <Toggle checked={rules.newCustomerAlert} onChange={(v) => setRules((r) => ({ ...r, newCustomerAlert: v }))} color="#10b981" />
          </div>
        </div>
      </div>
      <div className="flex items-center gap-3 flex-wrap">
        <button onClick={saveRules} disabled={saving} className="rounded-xl bg-[#10b981] px-5 py-2 font-mono text-xs font-bold text-white hover:bg-[#059669] transition disabled:opacity-60">
        ...
        <button onClick={clearRules} className="font-mono text-[10px] text-[#6a6a90] hover:text-red-400 transition ml-auto">Clear all</button>
      </div>
      <div className="flex items-center gap-3">
        <p className="font-mono text-[9px] text-[#58588a]">Alert thresholds are tied to your account and checked nightly.</p>
        {lastSaved
          ? <p className="font-mono text-[9px] text-[#58588a]">Last saved: {lastSaved}</p>
          : <p className="font-mono text-[9px] text-[#f59e0b]">Last saved: never</p>}
      </div>
    </div>
  );
}
...
function DigestSection({ email }: { email: string }) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between rounded-xl bg-white ring-1 ring-black/[0.06] px-4 py-3">
        <div>
          ...
          <p className="mt-0.5 font-mono text-[10px] text-[#6a6a90]">
            {subscribed
              ? digestFrequency === "daily" ? `Delivered every day to ${email}`
                : digestFrequency === "monthly" ? `Delivered on day ${digestDay} of each month`
                : `Delivered every ${DOW_LABELS[digestDay]} to ${email}`
              ...
              {(["daily", "weekly", "monthly"] as const).map((f) => (
                <button key={f}
                  onClick={() => {
                    ...
                    setDigestFrequency(f); setDigestDay(newDay); savePrefs(subscribed, newDay, f);
                  }}
                  className={`flex-1 rounded-lg px-3 py-2 font-mono text-[11px] font-semibold border transition-all ${digestFrequency === f ? "border-[#3b82f6]/40 bg-[#3b82f6]/10 text-[#3b82f6]" : "border-[#eeeef4] text-[#6a6a90] hover:text-[#4a4a6a] hover:border-black/15"}`}
                >
            ...
            {digestFrequency === "weekly" && (
              <div className="flex gap-1.5 flex-wrap">
                {DOW_LABELS.map((label, i) => (
                  <button key={i} onClick={() => { setDigestDay(i); savePrefs(subscribed, i, digestFrequency); }}
                    className={`rounded-lg px-3 py-1.5 font-mono text-[11px] font-semibold transition-all ${digestDay === i ? "bg-[#3b82f6]/15 border border-[#3b82f6]/40 text-[#3b82f6]" : "border border-[#eeeef4] text-[#6a6a90] hover:text-[#4a4a6a]"}`}
                  >
                    {label}
                  </button>
                ))}
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-1.5 flex-wrap">
                  {Array.from({ length: 28 }, (_, i) => i + 1).map((d) => (
                    <button key={d} onClick={() => { setDigestDay(d); savePrefs(subscribed, d, digestFrequency); }}
                      className={`rounded-lg w-8 h-8 font-mono text-[11px] font-semibold transition-all ${digestDay === d ? "bg-[#3b82f6]/15 border border-[#3b82f6]/40 text-[#3b82f6]" : "border border-[#eeeef4] text-[#6a6a90] hover:text-[#4a4a6a]"}`}
                    >
                      {d}
                    </button>
                  ))}
      ...
      <p className="font-mono text-[9px] text-[#58588a]">Last sent: <span className={lastSent ? "text-[#4a4a6a]" : "text-[#f59e0b]"}>{lastSent ?? "Never"}</span></p>
    </div>
  );
}
...
function NewsletterToggle() {
  ...
  async function toggle(next: boolean) {
    setEnabled(next); setSaving(true); setSaved(false);
    try {
      await fetch("/api/user/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className={`flex items-start justify-between gap-4 transition-opacity duration-300 ${loaded ? "opacity-100" : "opacity-0"}`}>
      <div className="min-w-0">
        ...
        <p className="mt-1 text-xs text-[#6a6a90]">Tips, new integrations, and product updates. Transactional emails (digest, alerts) are always delivered regardless.</p>
...
function GoalsSection({ currencies }: { currencies: Record<string, string> }) {
  ...
  async function saveGoals() {
    ...
    try {
      ...
      setSaved(true); setLastSaved("Just now");
  ...
  const KPI_ROWS: {
    key: keyof Goals; label: string; sublabel: string; unit: string; unitPos: "left" | "right";
    placeholder: string; color: string; icon: React.ReactNode;
    toDisplay: (v: number) => string; toStorage: (s: string) => number;
    progressCurrent?: number; progressGoal?: number; noDataMsg?: string;
  }[] = [
    {
      key: "revenueTarget", label: "Monthly revenue target", sublabel: `Goal line on Overview & AI Advisor · ${revCurrency}`,
      unit: revCurrency, unitPos: "left", placeholder: "1000", color: "#eab308",
      progressCurrent: 2000, progressGoal: goals.revenueTarget || 0,
      icon: <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6" /></svg>,
      toDisplay: (v) => (v ? (v / 100).toFixed(0) : ""), toStorage: (s) => (s ? Math.round(parseFloat(s) * 100) : 0),
    },
    {
      key: "sessionsTarget", label: "Monthly sessions target", sublabel: "Web traffic goal from GA4 / Plausible / PostHog",
      unit: "sessions", unitPos: "right", placeholder: "20000", color: "#6366f1",
      progressCurrent: 52, progressGoal: goals.sessionsTarget || 0,
      ...
      toDisplay: (v) => (v ? String(v) : ""), toStorage: (s) => (s ? parseInt(s) || 0 : 0),
    },
    {
      key: "subscribersTarget", label: "Monthly subscribers target", sublabel: "Email list growth from Mailchimp / Beehiiv / Klaviyo",
      unit: "subs", unitPos: "right", placeholder: "500", color: "#14b8a6",
    ...
    {
      key: "adSpendBudget", label: "Monthly ad spend budget", sublabel: `Max budget cap across Meta / Google / TikTok · ${adsCurrency}`,
      unit: adsCurrency, unitPos: "left", placeholder: "2000", color: "#f59e0b",
  ...
  if (!loaded) return <div className="space-y-3">{[1,2,3,4].map((i) => <div key={i} className="h-20 animate-pulse rounded-xl bg-[#f0f0f8]" />)}</div>;
  ...
  return (
    <div className="space-y-4">
            ...
            <div key={row.key} className="rounded-xl bg-white ring-1 ring-black/[0.06] overflow-hidden shadow-[0_1px_3px_rgba(0,0,0,0.05)]">
              <div className="p-4">
                <div className="flex items-start gap-3">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg mt-0.5" style={{ backgroundColor: `${row.color}15`, color: row.color }}>{row.icon}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-[#3a3a58]">{row.label}</p>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <input
                      type="number" min={0} placeholder={row.placeholder} value={displayVal}
                ...
                {numericGoal > 0 && !row.noDataMsg && (
                  <div className="mt-3 ml-11">
                    <div className="flex items-center justify-between mb-1">
                      <p className="font-mono text-[9px] text-[#6a6a90]">Progress: {pct}%</p>
                ...
                {row.noDataMsg && <p className="mt-2 ml-11 font-mono text-[9px] text-[#6a6a90] italic">💡 {row.noDataMsg}</p>}
              </div>
            </div>
          );
        })}
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <button onClick={saveGoals} disabled={saving} className="rounded-xl bg-[#eab308] px-5 py-2 font-mono text-xs font-bold text-white hover:bg-[#ca8a04] transition disabled:opacity-60">
        ...
        <button onClick={clearGoals} className="font-mono text-[10px] text-[#6a6a90] hover:text-red-400 transition ml-auto">Clear all</button>
      </div>
      <div className="flex items-center gap-3">
...
const NAV_ITEMS = [
  { id: "integrations", label: "Integrations",  color: "#14b8a6", icon: <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71" /><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71" /></svg> },
  { id: "goals",        label: "Goals & KPIs",  color: "#eab308", icon: <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><path d="M12 8v4l3 3" /></svg> },
  { id: "alerts",       label: "Alert Rules",   color: "#ef4444", icon: <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0" /></svg> },
  { id: "email",        label: "Email Digest",  color: "#3b82f6", icon: <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg> },
  { id: "preferences",  label: "Preferences",   color: "#8b5cf6", icon: <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 012.83-2.83l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z" /></svg> },
];

// ── Main Component ────────────────────────────────────────────────────────

export default function DataSourcesTab({ email, isPremium, connectedPlatforms, currencies, isDemo = false }: DataSourcesTabProps) {
  const router = useRouter();
  ...
  useEffect(() => {
    ...
    if (hasJunkParams) {
      ...
      router.replace("/dashboard?tab=data-sources");
    }
  ...
  function closeConnectModal() {
    setConnectTarget(null); setConnectError(""); setConnectSuccess("");
    router.replace("/dashboard?tab=data-sources");
  }

  async function submitConnect(platform: string, payload: Record<string, string>) {
    setConnectLoading(true); setConnectError(""); setConnectSuccess("");
    try {
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full" style={{ background: "#f5f5f7" }}>
      ...
      {/* ── Left sidebar ──────────────────────────────────────────────── */}
      <aside className="hidden lg:flex flex-col w-[200px] shrink-0 sticky top-0 h-screen pt-8 pb-6 border-r border-[#eeeef4]">
        {/* Brand mark */}
        <div className="px-5 mb-6">
          <div className="flex items-center gap-2.5">
            <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#14b8a6]/10">
              <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="#14b8a6" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round">
                <ellipse cx="12" cy="5" rx="9" ry="3" /><path d="M3 5v6c0 1.66 4.03 3 9 3s9-1.34 9-3V5" /><path d="M3 11v6c0 1.66 4.03 3 9 3s9-1.34 9-3v-6" />
              </svg>
            </div>
            <div>
              <p className="font-mono text-[11px] font-bold text-[#1a1a2e]">Data Sources</p>
              <p className="font-mono text-[9px] text-[#6a6a90]">{connectedCount} of {totalIntegrations} connected</p>
            </div>
          </div>
          {/* Progress bar */}
          <div className="mt-3 h-1 w-full rounded-full bg-[#e8e8f4] overflow-hidden">
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <button
                ...
                className="relative flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-left transition-all group"
              ...
              >
                ...
                <span style={{ color: isActive ? item.color : "#9090b0" }} className="transition-colors group-hover:opacity-100">
                  {item.icon}
                </span>
                <span className="font-mono text-[12px] font-medium transition-colors" style={{ color: isActive ? item.color : "#6a6a90" }}>
                  {item.label}
                </span>
              </button>
            );
        ...
        <div className="mt-auto px-4">
          {isPremium ? (
            <div className="rounded-xl border border-[#10b981]/20 bg-[#10b981]/5 px-3 py-2.5 space-y-1">
              <div className="flex items-center gap-1.5">
                <span className="h-1.5 w-1.5 rounded-full bg-[#10b981] animate-pulse" />
                <span className="font-mono text-[9px] font-semibold text-[#10b981]">Premium Active</span>
              </div>
              <p className="font-mono text-[9px] text-[#6a6a90]">Alerts &amp; digest enabled</p>
            </div>
          ) : (
            <div className="rounded-xl border border-[#f59e0b]/20 bg-[#f59e0b]/5 px-3 py-2.5">
              <p className="font-mono text-[9px] font-semibold text-[#f59e0b]">Alerts &amp; Digest</p>
              <p className="font-mono text-[9px] text-[#6a6a90] mt-0.5">Require Premium plan</p>
            </div>
          )}
        </div>
      </aside>
      ...
      {/* ── Mobile pill nav ───────────────────────────────────────────── */}
      <div className="lg:hidden sticky top-0 z-10 flex gap-1.5 overflow-x-auto scrollbar-none border-b border-[#eeeef4] px-4 py-2.5" style={{ background: "#f5f5f7" }}>
        {NAV_ITEMS.map((item) => {
          const isActive = activeSection === item.id;
          return (
            <button key={item.id} onClick={() => scrollTo(item.id)}
              className="shrink-0 flex items-center gap-1.5 rounded-full px-3 py-1.5 font-mono text-[11px] font-semibold border transition-all whitespace-nowrap"
              style={{
        ...
        <div className="mx-auto max-w-[820px] px-6 lg:px-8 space-y-10 pt-8">
          ...
          <div className="hidden lg:block">
            <div className="flex items-center gap-3 mb-1">
              <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#14b8a6]/10">
                <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#14b8a6" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round">
                  <ellipse cx="12" cy="5" rx="9" ry="3" /><path d="M3 5v6c0 1.66 4.03 3 9 3s9-1.34 9-3V5" /><path d="M3 11v6c0 1.66 4.03 3 9 3s9-1.34 9-3v-6" />
                </svg>
              </div>
              <h1 className="font-mono text-lg font-bold text-[#1a1a2e]">Data Sources & Notifications</h1>
            </div>
            <p className="text-sm text-[#6a6a90] ml-11">Manage integrations, targets, alert rules and digest preferences.</p>
          </div>
          ...
          <section id="integrations" ref={(el) => { sectionRefs.current.integrations = el; }}>
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#14b8a6]/10 text-[#14b8a6]">
                <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71" /><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71" /></svg>
              </div>
              <div>
                <h2 className="font-mono text-sm font-bold text-[#1a1a2e]">Integrations</h2>
                <p className="font-mono text-[10px] text-[#6a6a90]">Connect your data sources · syncs every 24h</p>
              </div>
              <div className="ml-auto flex items-center gap-2">
                <div className="h-1.5 w-16 rounded-full bg-[#e8e8f4] overflow-hidden">
                  <div className="h-full rounded-full bg-[#14b8a6] transition-all" style={{ width: `${(connectedCount / totalIntegrations) * 100}%` }} />
                </div>
                <span className="font-mono text-[10px] text-[#6a6a90]">{connectedCount}/{totalIntegrations}</span>
              </div>
            </div>

            <div className="rounded-2xl bg-white ring-1 ring-black/[0.06] p-6 shadow-[0_1px_4px_rgba(0,0,0,0.05)] space-y-5">
              {/* Search + Category */}
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="relative flex-1">
                  ...
                  <input
                    type="text" placeholder="Search platforms…" value={searchQuery}
                    ...
                    return (
                      <div key={cat} className="space-y-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="h-px flex-1 bg-[#f0f0f8]" />
                        </div>
              ...
              {!searchQuery && activeCategory === "Popular" && (
                <div className="border-t border-[#f0f0f8] pt-5">
                  <p className="font-mono text-[10px] font-bold uppercase tracking-widest text-[#58588a] mb-3">Coming Soon</p>
                  <ComingSoonSection />
                </div>
              )}
            </div>
          ...
          <section id="goals" ref={(el) => { sectionRefs.current.goals = el; }}>
            <div className="flex items-center gap-3 mb-4">
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#eab308]/10 text-[#eab308]">
                <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><path d="M12 8v4l3 3" /></svg>
              </div>
              <div>
                <h2 className="font-mono text-sm font-bold text-[#1a1a2e]">Goals & KPIs</h2>
                <p className="font-mono text-[10px] text-[#6a6a90]">Monthly targets that power forecast bars &amp; AI analysis</p>
              </div>
            </div>
          ...
          <section id="alerts" ref={(el) => { sectionRefs.current.alerts = el; }}>
            <div className="flex items-center gap-3 mb-4">
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#ef4444]/10 text-[#ef4444]">
                <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0" /></svg>
              </div>
              <div>
                <h2 className="font-mono text-sm font-bold text-[#1a1a2e]">Alert Rules</h2>
                <p className="font-mono text-[10px] text-[#6a6a90]">Get notified when key metrics cross thresholds</p>
              </div>
              {!isPremium && (
                <span className="ml-auto inline-flex items-center gap-1 rounded-full border border-[#f59e0b]/30 bg-[#f59e0b]/8 px-2.5 py-1 font-mono text-[9px] font-semibold text-[#f59e0b]">
            ...
            {isPremium ? (
              ...
              <div className="rounded-2xl bg-white ring-1 ring-black/[0.06] p-6 shadow-[0_1px_4px_rgba(0,0,0,0.05)] flex items-center gap-4">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#f59e0b]/10 text-[#f59e0b]">
                  <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9M13.73 21a2 2 0 01-3.46 0" /></svg>
                </div>
                <div>
                  <p className="text-sm font-semibold text-[#1a1a2e]">Alert Rules — Premium feature</p>
                  <p className="text-xs text-[#6a6a90] mt-0.5">Upgrade to Premium to configure alert thresholds and receive email notifications.</p>
                </div>
              </div>
            )}
          </section>
          ...
          <section id="email" ref={(el) => { sectionRefs.current.email = el; }}>
            <div className="flex items-center gap-3 mb-4">
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#3b82f6]/10 text-[#3b82f6]">
                <svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round"><path d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
              </div>
              <div>
                <h2 className="font-mono text-sm font-bold text-[#1a1a2e]">Email Digest</h2>
                <p className="font-mono text-[10px] text-[#6a6a90]">AI summary delivered on your chosen schedule</p>
              </div>
              {!isPremium && (
                <span className="ml-auto inline-flex items-center gap-1 rounded-full border border-[#f59e0b]/30 bg-[#f59e0b]/8 px-2.5 py-1 font-mono text-[9px] font-semibold text-[#f59e0b]">
                  <svg width="9" height="9" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" /></svg>
            ...
            {isPremium ? (
              <div className="rounded-2xl bg-white ring-1 ring-black/[0.06] p-6 shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
                <DigestSection email={email} />
              </div>
            ) : (
              <div className="rounded-2xl bg-white ring-1 ring-black/[0.06] p-6 shadow-[0_1px_4px_rgba(0,0,0,0.05)] flex items-center gap-4">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#3b82f6]/10 text-[#3b82f6]">
                  <svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round"><path d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                </div>
                <div>
                  <p className="text-sm font-semibold text-[#1a1a2e]">Email Digest — Premium feature</p>
                  <p className="text-xs text-[#6a6a90] mt-0.5">Upgrade to Premium to receive AI-generated digest emails.</p>
                </div>
              </div>
            )}
          </section>
          ...
          <section id="preferences" ref={(el) => { sectionRefs.current.preferences = el; }}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="font-mono text-sm font-bold text-[#1a1a2e]">Preferences</h2>
                <p className="font-mono text-[10px] text-[#6a6a90]">Newsletter &amp; communication settings</p>
              </div>
            </div>
            <div className="rounded-2xl bg-white ring-1 ring-black/[0.06] p-6 shadow-[0_1px_4px_rgba(0,0,0,0.05)]">
              <NewsletterToggle />
        ...
        >
          ...
          >
            {DYNAMIC_MODALS[connectTarget].map((field) => (
              <div key={field.name}>
                <input
                  name={field.name} required={!field.optional}
            ...
            <button disabled={connectLoading} className="rounded-xl bg-[#6366f1] px-4 py-2 font-mono text-xs font-bold text-white hover:bg-[#4f46e5] disabled:opacity-60 transition-all">
