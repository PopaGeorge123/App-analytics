...
import type { AiPlaybook, AiPlaybookStep } from "@/app/api/ai/playbooks/route";

function FoldLogo({ height = 28 }: { height?: number }) {
  const aspect = 120 / 52;
...
async function getPlaybook(token: string): Promise<{ payload: SharePayload; expiresAt: string; viewCount: number } | null> {
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000";
  const res = await fetch(`${baseUrl}/api/report/${token}`, { cache: "no-store" });
  if (!res.ok) return null;
  const data = await res.json();
  if (data.payload?.type !== "playbook") return null;
  return { payload: data.payload as SharePayload, expiresAt: data.expiresAt, viewCount: data.viewCount };
}
...
const SEVERITY_STYLES: Record<string, { bg: string; border: string; text: string; dot: string }> = {
  ...
  warning:     { bg: "#f59e0b12", border: "#f59e0b30", text: "#fbbf24", dot: "#f59e0b" },
  opportunity: { bg: "#10b98112", border: "#10b98130", text: "#34d399", dot: "#10b981" },
};
...
export default async function PublicPlaybookPage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const result = await getPlaybook(token);
  if (!result) notFound();

  const { payload, expiresAt, viewCount } = result;
  const { playbook, sharedBy } = payload;
  const sev = SEVERITY_STYLES[playbook.severity] ?? SEVERITY_STYLES.opportunity;
    ...
    <div style={{ minHeight: "100vh", background: "#09090f", fontFamily: "'Inter', system-ui, sans-serif", padding: "0" }}>
      ...
      <div style={{ borderBottom: "1px solid #303040", background: "#21212c", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <span style={{ fontFamily: "monospace", fontSize: 10, color: "#58588a" }}>
            {viewCount} view{viewCount !== 1 ? "s" : ""} · Expires {new Date(expiresAt).toLocaleDateString()}
          </span>
          <a
            href="/signup"
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 5, borderRadius: 6, background: sev.bg, border: `1px solid ${sev.border}`, padding: "3px 10px", fontFamily: "monospace", fontSize: 11, fontWeight: 700, color: sev.text }}>
          ...
          <span style={{ borderRadius: 6, background: catColor + "18", border: `1px solid ${catColor}30`, padding: "3px 10px", fontFamily: "monospace", fontSize: 11, fontWeight: 600, color: catColor }}>
            {playbook.category}
          </span>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 32 }}>
          <div style={{ borderRadius: 12, border: "1px solid #10b98130", background: "#10b98108", padding: "16px 20px" }}>
            <p style={{ fontFamily: "monospace", fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.12em", color: "#10b981", marginBottom: 8 }}>Expected Gain</p>
            <p style={{ fontSize: 14, fontWeight: 600, color: "#e2e8f0", lineHeight: 1.5 }}>{playbook.expectedGain}</p>
          </div>
          <div style={{ borderRadius: 12, border: "1px solid #6366f130", background: "#6366f108", padding: "16px 20px" }}>
            <p style={{ fontFamily: "monospace", fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.12em", color: "#6366f1", marginBottom: 8 }}>Impact</p>
            <p style={{ fontSize: 14, fontWeight: 600, color: "#e2e8f0", lineHeight: 1.5 }}>{playbook.impact}</p>
          </div>
        </div>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {step.link && (
                    <a
                      ...
                      style={{ display: "inline-flex", alignItems: "center", gap: 5, marginTop: 8, fontFamily: "monospace", fontSize: 11, color: "#6366f1", textDecoration: "none" }}
                    >
                      {step.link.label} →
                    </a>
                  )}
